...
- **An optional conflict check, every turn.** When enabled, rumination is
  ...
  turn, and why, tied to their goals/canon/approval — scaled by approval, so
  the default flips to genuine warmth/compliance once it's earned. Generic
  "don't be a yes-man" guidance is easy for a model to drop under context
  pressure; a required per-turn field isn't. It's a boundary, not a script —
  it says what the character withholds, never the line or action they use to
  withhold it. **Off by default** ("Per-turn conflict check" in settings) —
