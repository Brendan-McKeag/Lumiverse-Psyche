...
export function setup(ctx: SpindleFrontendContext) {
  ...
  function emotionRow(e: Emotion): string {
    ...
    return `<div class="ps-emo"><span class="nm" title="${esc(e.label)}">${esc(e.label.split(' (')[0])}</span>${bar}<span class="ps-val">${valTxt} ${esc(
      e.descriptor,
    )}</span></div>`
  }
  ...
  function renderDetail() {
    ...
    // bipolar axes first, then unipolar sorted by intensity
